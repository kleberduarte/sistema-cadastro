...
- **Frontend:** URL da API centralizada em `sistema-cadastro/sistema-cadastro-clientes/api-config.js` — **ajuste esse arquivo** (ou injete `window.API_URL` antes dele) no ambiente real. *(Se ainda não rodou `organizar-sistema-cadastro.bat`, o front pode estar em `sistema-cadastro-clientes/api-config.js` na raiz do repositório.)*
